...
class DynamicMVNet(Base3DDetector):
    ...
    def __init__(self,
        ...
        super(DynamicMVNet, self).__init__()
        ...
        if img_rpn_head is not None:
            img_rpn_head_ = img_rpn_head.copy() # why copy?
            img_rpn_head_.update(test_cfg=test_cfg.img_rpn)
            if train_cfg is not None:
                img_rpn_head_.update(train_cfg=train_cfg.img_rpn)
            self.img_rpn_head = builder.build_head(img_rpn_head_)
        if img_roi_head is not None:
            img_roi_head_ = img_roi_head.copy()
            img_roi_head_.update(test_cfg=test_cfg.img_rcnn)
            if train_cfg is not None:
                img_roi_head_.update(train_cfg=train_cfg.img_rcnn)
            self.img_roi_head = builder.build_head(img_roi_head_)

        # init weight for img branch
    ...
    def init_weights(self, pretrained=None):
        super(DynamicMVNet, self).init_weights()
    ...
    def extract_img_feat(self, img, img_metas):
        if img is not None and img.dtype == torch.uint8:
            img = img.float()
    ...
    def extract_pts_feat(self, points, img_feats, img_metas):
        ...
        voxel_features, feature_coors = self.pts_voxel_encoder(\
                voxels, coors, img_feats=img_feats, img_metas=img_metas, points=points)
        batch_size = coors[-1, 0].item() + 1
        middle_encode_features = self.pts_middle_encoder(voxel_features, feature_coors, batch_size)
        x = self.pts_backbone(middle_encode_features)
        if self.with_pts_neck:
            x = self.pts_neck(x)
        return x, middle_encode_features
        
    def extract_feat(self, points, img, img_metas):
        """Extract features from images and points."""
        if img is not None and self.with_img_backbone:
            img_feats = self.extract_img_feat(img, img_metas)
        else:
            img_feats = None
        if self.with_pts_backbone == False:
            pts_feats = None
        else:
            pts_feats, middle_encode_features = self.extract_pts_feat(points, img_feats, img_metas)
    ...
    def forward_train(self,
        ...
        """
        # NOTE visualize cut & paste img in vis/
        # print("Visualize Img")
        # import cv2
        # import numpy as np
        # print(img.shape)
        # mean = np.array([103.530, 116.280, 123.675]).reshape(1, 1, 3)
        # cv2.imwrite('plot_fig/cut_paste.png', img.permute(0, 2, 3, 1)[0].cpu().numpy() + mean)
        # exit()
    ...
    def forward_img_train(self,
        ...
        if self.with_img_roi_head:
            roi_losses = self.img_roi_head.forward_train(x, img_metas, proposal_list,
                                                    gt_bboxes, gt_labels,
                                                    gt_bboxes_ignore,
                                                    **kwargs)
            # NOTE avoid 3d name conflicts
            roi_losses['loss_cls_2d'] = roi_losses['loss_cls']
            roi_losses['loss_bbox_2d'] = roi_losses['loss_bbox']
            del roi_losses['loss_cls']
            del roi_losses['loss_bbox']
            losses.update(roi_losses)
        return losses

    def simple_test_img(self, x, img_metas, proposals=None, rescale=False, debug_imgs=None):
        ...
        if proposals is None:
            ...
            proposal_list = proposals
        # plot_img = debug_imgs[0].permute(1, 2, 0).contiguous().cpu().numpy()
        # from plot_fig.plot_utils import draw_bbox
        # import cv2
        # for each_bbox in proposal_list[0].cpu().numpy():
        #     if each_bbox[-1] > 0.5:
        #         plot_img = draw_bbox(plot_img, each_bbox, 1, show_score=False)
        # cv2.imwrite('plot_fig/tmp.png', plot_img)
        # exit()
        # print(debug_imgs[0].shape, proposal_list[0].shape)
    ...
    def simple_test(self, points, img_metas, img=None, rescale=False):
        ...
        if img_feats and self.with_img_roi_head:
            bbox_img = self.simple_test_img(
                img_feats, img_metas, rescale=rescale, debug_imgs=img)
    ...
    def aug_test(self, points, img_metas, imgs=None, rescale=False):
        ...
        img_feats, pts_feats, pillar_feat, new_img_feats = self.extract_feats(points, img_metas=img_metas, imgs=imgs)
        bbox_list = dict()
        if pts_feats and self.with_pts_bbox:
    ...
    def extract_feats(self, points, img_metas, imgs=None):
        ...
        img_feats, pts_feats, pillar_feats, new_img_feats = multi_apply(self.extract_feat, points, imgs,
                                           img_metas)
        return img_feats, pts_feats, pillar_feats, new_img_feats
