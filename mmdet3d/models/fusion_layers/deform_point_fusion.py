...
def get_reference_points(img_meta,
                 img_features,
                 points,
                 proj_mat,
                 coord_type,
...
class DeformPointFusion(BaseModule):
    ...
    def __init__(self,
                 ...
                 padding_mode='zeros',
                 lateral_conv=True):
        super(DeformPointFusion, self).__init__(init_cfg=init_cfg)
        ...
        self.pts_key_proj = nn.Sequential(
            nn.Linear(pts_channels, mid_channels),
    ...
    def forward(self, img_feats, pts, pts_feats, img_metas):
        ...
        """
        img_pts = self.obtain_mlvl_feats(img_feats, pts, pts_feats, img_metas)
    ...
    def obtain_mlvl_feats(self, img_feats, pts, pts_feats, img_metas):
    ...
    def sample_single(self, img_feats, pts, pts_feats, img_meta, level_num):
        ...
        flatten_img_feat = img_feats.permute(0, 2, 3, 1).reshape(bs, h * w, channel_num)   # (n, H x W, C)
        ref_points = ref_points.reshape(bs, -1, 1, 2)       # (n, num_query, n_levels, 2)
        ...
        img_pts = self.deform_layers[level_num](pts_feats, ref_points, flatten_img_feat, level_spatial_shapes, level_start_index).squeeze(0)
        invalid_idx = ((ref_points[:, :, :, 0] >= 0) & (ref_points[:, :, :, 1] >= 0) \
                    & (ref_points[:, :, :, 0] <= 1) & (ref_points[:, :, :, 1] <= 1)).squeeze()
        img_pts[invalid_idx] = 0.
        return img_pts
