...
class MultiVoxelFusionFastAdd(BaseModule):
    ...
    def __init__(self,
                 ...
                 padding_mode='zeros',
                 lateral_conv=True):
        super(MultiVoxelFusionFastAdd, self).__init__(init_cfg=init_cfg)
            ...
            for level in range(len(self.img_levels)):
                mlvl_img_feats.append(
                    self.sample_single(img_ins[level][i * num_camera: \
                            (i + 1) * num_camera], decoded_voxel_coors, img_metas[i]))
    ...
    def sample_single(self, img_feats, pts, img_meta):
        ...
        for camera_id in range(num_camera):
            ...
            final_img_pts[assign_idx] = point_features.squeeze().t()
